...
export async function generatePDF(data: PartWithDetails[], title: string, stats?: any) {
  const doc = new jsPDF();
...
export async function generateSupplierAnalysisPDF(data: any[], dateRange?: { from: Date; to: Date }) {
  const doc = new jsPDF();
...
export async function generateStockMovementsPDF(movements: any[]) {
  const doc = new jsPDF();
...
// Low Stock Report
export async function generateLowStockPDF(data: any[]) {
  const doc = new jsPDF();
