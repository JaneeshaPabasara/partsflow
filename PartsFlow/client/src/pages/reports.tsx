...
export default function Reports() {
  ...
  const { data: parts } = useQuery<PartWithDetails[]>({ queryKey: ["/api/parts"] });
  const { data: lowStockParts } = useQuery<PartWithDetails[]>({ queryKey: ["/api/parts/low-stock"] });
  const { data: movements = [] } = useQuery<any[]>({ queryKey: ["/api/movements"] });
  const { data: suppliers = [] } = useQuery<any[]>({ queryKey: ["/api/suppliers"] });
  const { data: recentReports = [] } = useQuery<Report[]>({ queryKey: ["/api/reports"] });
  ...
  const createReport = async (type: string, name: string) => {
            ...
            {recentReports.length === 0 ? (
              ...
              <div className="space-y-2">
                {recentReports.slice(0, 5).map((report) => (
                  <div key={report.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg" data-testid={`report-item-${report.id}`}>
                    <div>
                      <p className="font-medium text-slate-900" data-testid={`report-name-${report.id}`}>
                        {report.name}
                      </p>
                      <p className="text-sm text-slate-600" data-testid={`report-date-${report.id}`}>
              ...
              <Card key={report.id} className="hover:shadow-md transition-shadow">
                <CardHeader>
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-slate-900" data-testid={`text-report-title-${report.id}`}>
                        {report.title}
                      </h3>
                      <p className="text-sm text-slate-600" data-testid={`text-report-desc-${report.id}`}>
                        {report.description}
                      </p>
                    </div>
                  </div>
